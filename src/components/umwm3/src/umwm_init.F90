...
stopTimeStr,dtg,restart,hmax_limiter,dt_relax
...
sdt_fac,sbf_fac,sbp_fac,hmax_coef
...
ELSEIF(option==2)THEN
  ...
  ALLOCATE(slim(istart:iend)) ! timestep related to variance spec. growth
    ...
    OUTER2:DO n=1,nm
      INNER2:DO m=1,mm
        ...
        IF(istart==i)THEN
          ...
          IF(COUNT(mask(1:m-1,n)==1)==0)extend=0  ! BK fix for partitioning bug.
    ...
    OUTER4:DO m=1,mm
      INNER4:DO n=1,nm
        ...
        IF(istart==i)THEN
          ...
          IF(COUNT(mask(m,1:n-1)==1)==0)extend=0  ! BK fix for partitioning bug.
          istart = istart+extend
          EXIT OUTER4
        ENDIF
      ENDDO INNER4
