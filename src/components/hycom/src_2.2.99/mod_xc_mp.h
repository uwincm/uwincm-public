...
c**********
c*
c  1) broadcast an entire 2-D non-tiled array
c
c  2) For zaiordg only.
c     arrays are real*4 with no halo.
c
c  3) mnflg selects which nodes must contain the input non-tiled array
c        =-1; first node in each row contains that row
c        = n; node number n (mnproc=n)
c     the array aa is unchanged on exit, and need not exist
c      on the nodes that do not reference it.
...
c       name            type         usage            description
c    ----------      ----------     -------  ----------------------------
c    aa              real*4         input     non-tiled source array
c    ga              real*4         output    non-tiled target array
c    mnflg           integer        input     node source flag
c*
c**********
c
