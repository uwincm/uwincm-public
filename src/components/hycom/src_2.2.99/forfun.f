...
c**********
c*
c  1) convert date into 'model day', for yrflag=2,3 only.
c
c  2) for yrflag==3, the 'model day' is the number of days since 
c     001/1901 (which is model day 1.0).
c     for example:
...
c
c  3) for yrflag==2, all years are leap years.
c     for example:
c      a) iyr=1,mon=1,idy=1 is the 1st day of the run,
c         so wday would be 1.0.
c      a) iyr=1,mon=1,idy=2 is the 2nd day of the run,
c         so wday would be 2.0.
c*
c**********
c
...
!$OMP&           SCHEDULE(STATIC,jblk)
        do j= 1,jj
          do i= 1,ii
c ---       veldf[24]u are veldf[24]p, and veldf[24]v not used
            if     (ip(i,j).eq.1) then
...
c> July 2017 - always define rivers array
c> July 2017 - datefor and lmonth now also for yrflag=2.
c> July 2017 - for montyp=3,4 veldf[24]u is actually veldf[24]p
