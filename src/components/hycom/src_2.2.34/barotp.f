...
c
              pbaros(i,j) = (1.-2.*wts2)*pbavg(i,j,m)+wts2*pbavg(i,j,n)
              ubaros(i,j) = (1.-2.*wts2)*ubavg(i,j,m)+wts2*ubavg(i,j,n)
              vbaros(i,j) = (1.-2.*wts2)*vbavg(i,j,m)+wts2*vbavg(i,j,n)
            enddo !i
          enddo !j
        else !1st time step
...
!$OMP&           SCHEDULE(STATIC,jblk)
        do j=1,jj
          do i=1,ii
            ubavg(i,j,m) = ubaros(i,j)+wts2*ubavg(i,j,n)
            vbavg(i,j,m) = vbaros(i,j)+wts2*vbavg(i,j,n)
            pbavg(i,j,m) = pbaros(i,j)+wts2*pbavg(i,j,n)
          enddo !i
        enddo !j
...
c> Apr. 2010 - added Asselin filtering to btrlfr
