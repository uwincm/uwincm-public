...
c
     ...
     &        sumdp,sumth
      integer i,j,k,kl,l,mbdy,hlstep
c
*     real*8    wtime
*     external  wtime
...
c ---           average currents over top dp00 (layer 1 in deep water). 
                usur = 0.0
                vsur = 0.0
                psur = 0.0
                do k= 1,max(nsigma,1) !near surface can't be deeper than nsigma
                  dpsur = min( dp(i,j,k,n), max( 0.0, dp00 - psur ) )
...
c
           do 161 i=max(1-nbdy+1,ifu(j,l)-1),min(ii+nbdy,ilu(j,l)+2)
...
c
     ...
     &                    )
c
             visc(i,j)=max(veldf4u(i,j),velmodul*facdf4,visco4*defortot)
c ---        CFL constraint on 2D explicit biharmonic viscosity
             visc(i,j)=min(cflp(i,j),cflp(i-1,j), visc(i,j) )
 163       continue
c
...
c
 ...
 164       continue
c
c ---     restore uwk from boundary conditions : begin
          i=ifu(j,l)
          if     (i-2.ge. 1-nbdy) then
            uwkn(i-1,j)=utotn(i-1,j)
            uwkn(i-2,j)=utotn(i-2,j)
            uwkm(i-1,j)=utotm(i-1,j)
            uwkm(i-2,j)=utotm(i-2,j)
          endif
          i=ilu(j,l)
          if     (i+2.le.ii+nbdy) then
            uwkn(i+1,j)=utotn(i+1,j)
...
c ---     restore uwk from boundary conditions : end
   16 continue
!$OMP END PARALLEL DO
c
c
...
c 
     ...
     &              ivopn(i-1,j+1).eq.0      ) then
              advu(i,j)=advu(i,j)+
     &         0.25*(vwkm(i-1,j)+vwkm(i-1,j+1)+vwkm(i,j)+vwkm(i,j+1))
     &             *scuxi(i,j)*(duadvjp1-q12*(crmm-clpm))
            endif
c
            jp1=j+1
            if(iu(i,j+1).eq.0) then
...
!$OMP&         SCHEDULE(STATIC,jblk)
      do 17 j=1-margin,jj+margin
        do 17 l=1,isv(j)
c
c ---     modify vwk for boundary conditions : begin
          i=ifv(j,l)
          if     (i-2 .gt. 1-nbdy) then
            vwkn(i-1,j)=2.0*vwkn(i,  j)-vwkn(i+1,j)
            vwkn(i-2,j)=2.0*vwkn(i-1,j)-vwkn(i,  j)
            vwkm(i-1,j)=2.0*vwkm(i,  j)-vwkm(i+1,j)
            vwkm(i-2,j)=2.0*vwkm(i-1,j)-vwkm(i,  j)
c
c ---       Open Boundary Conditions
c
            if     ( iuopn(i,j  ).ne.0 .or.
     &               iuopn(i,j-1).ne.0     ) then
              vwkn(i-1,j)=vwkn(i,j)
              vwkn(i-2,j)=vwkn(i-1,j)
              vwkm(i-1,j)=vwkm(i,j)
              vwkm(i-2,j)=vwkm(i-1,j)
            endif 
          endif 
c
          i=ilv(j,l)
...
c ---       Open Boundary Conditions
c
            if     (iuopn(i+1,j  ).ne.0 .or.
     &              iuopn(i+1,j-1).ne.0     ) then
              vwkn(i+1,j)=vwkn(i,  j)
              vwkn(i+2,j)=vwkn(i+1,j)
              vwkm(i+1,j)=vwkm(i,  j)
              vwkm(i+2,j)=vwkm(i+1,j)
...
c ---     modify vwk for boundary conditions : end
c
          do 171 i=max(1-nbdy+1,ifv(j,l)-1),min(ii+nbdy,ilv(j,l)+2)
...
c
     ...
     &                   )
c
            visc(i,j)=max(veldf4v(i,j),velmodul*facdf4,visco4*defortot)
c ---       CFL constraint on 2D explicit biharmonic viscosity
            visc(i,j)=min(cflp(i,j),cflp(i,j-1), visc(i,j) )
 173      continue
...
c
  ...
  174     continue
c
c ---     restore vwk from boundary conditions : begin
          i=ifv(j,l)
          if     (i-2 .gt.  1-nbdy) then
            vwkn(i-1,j)=vtotn(i-1,j)
            vwkn(i-2,j)=vtotn(i-2,j)
            vwkm(i-1,j)=vtotm(i-1,j)
            vwkm(i-2,j)=vtotm(i-2,j)
          endif
          i=ilv(j,l)
          if     (i+2 .lt. ii+nbdy) then
            vwkn(i+1,j)=vtotn(i+1,j)
...
c ---     restore vwk from boundary conditions : end
   17 continue
!$OMP END PARALLEL DO
c
c -------------------------------------
