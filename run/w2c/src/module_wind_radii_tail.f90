...
subroutine get_thresh_radii(x, y, field, xc, yc, thresh, minmax, rad1, rad2, rad3, rad4, percentile)
...
logical::keeplooking
real,dimension(:),allocatable::dumarray
integer::nr
real :: percentile
...
if (count(  feature .eq. 1 .and. x .gt. xc .and. y .gt. yc ) .gt. 0) then
   ...
   nr=count(  feature .eq. 1 .and. x .gt. xc .and. y .gt. yc ) 
   allocate(dumarray(nr))

   dumarray= pack( dist ,   feature .eq. 1 .and. x .gt. xc .and. y .gt. yc  )

   call bubblesort ( dumarray, nr) 
   call throwpercentile ( dumarray, nr, percentile ,rad1)
...
if (count(  feature .eq. 1 .and. x .lt. xc .and. y .gt. yc ) .gt. 0)  then
   ...
   nr=count(  feature .eq. 1 .and. x .lt. xc .and. y .gt. yc  ) 
   allocate(dumarray(nr))

   dumarray= pack( dist ,  feature .eq. 1 .and. x .lt. xc .and. y .gt. yc    )
   call bubblesort ( dumarray, nr) 
   call throwpercentile ( dumarray, nr, percentile ,rad4)
   deallocate(dumarray)
...
if (count(  feature .eq. 1 .and. x .lt. xc .and. y .lt. yc ) .gt. 0)  then
   ...
   nr=count(  feature .eq. 1 .and. x .lt. xc .and. y .lt. yc ) 
   allocate(dumarray(nr))

   dumarray= pack( dist ,   feature .eq. 1 .and. x .lt. xc .and. y .lt. yc  )
   call bubblesort ( dumarray, nr) 
   call throwpercentile ( dumarray, nr, percentile ,rad3)
   deallocate(dumarray)
...
if (count(  feature .eq. 1 .and. x .gt. xc .and. y .lt. yc ) .gt. 0)   then
   ...
   nr=count(  feature .eq. 1 .and. x .gt. xc .and. y .lt. yc ) 
   allocate(dumarray(nr))

   dumarray= pack( dist ,  feature .eq. 1 .and. x .gt. xc .and. y .lt. yc  )
   call bubblesort ( dumarray, nr) 
   call throwpercentile ( dumarray, nr, percentile ,rad2)
