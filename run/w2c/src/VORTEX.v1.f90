...
!-------------------------------------------------------------------------------
       ...
       data TCscale /'TS','TS','C1','C2','C3','C4','C5'/ 
      write (*, *)
...
!        print*,'test1',min(slp,1)
        call get_slp         (ix, jx, kx, STATE, BASE, slp)
!        print*,'test2',min(slp,1000000)
...
!--- cyl ----- do the smoothing twice ---
        ...
        call wspd_smoothing_2d(uu10,ix,jx)
        call wspd_smoothing_2d(uu10,ix,jx)
        call wspd_smoothing_2d(uu10,ix,jx)
        call wspd_smoothing_2d(uu10,ix,jx)
        call wspd_smoothing_2d(uu10,ix,jx)
...
!       WSPD10 = SQRT( DIAG%u10**2 + DIAG%v10**2 )
       ...
       do i = 1,7 
          if ( wspd_max(tx) .ge. real(wind_thresh(i)/1.95) ) then
             call get_thresh_radii( BASE%xlong,BASE%xlat,WSPD10,lon_center(tx),lat_center(tx),real(wind_thresh(i)/1.95),0,rad1(i),rad2(i),rad3(i),rad4(i) )
          else
             rad1(i)=0.0
             rad2(i)=0.0
             rad3(i)=0.0
      ...
      do t = 1, tx
            if (is_found(t)) then
               ...
               if ( wspd_max(tx) .ge. real(wind_thresh(1)) ) then
                  ...
                  do i = 1,7
                     if ( wspd_max(tx) .ge. real(wind_thresh(i)) ) then
                        write(50,500) basin,',',0,',',get_date(time_str(1)),',','  ',',',runmodel,',', &
       &                       (int(lat_center(t)*10.0)),latmark,',',int((lon_center(t)*10.0)),lonmark, &
       &                       ',',int(wspd_max(t)),',',int(slp_min(t)),',',TCscalename,',',wind_thresh(i),',','NEQ',',',int(rad1(i)),',',&
       &                       int(rad2(i)),',',int(rad3(i)),',',int(rad4(i)),',',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,'  
                     endif
                  enddo
               else
                 write(50,500)basin,',',0,',',get_date(time_str(1)),',','  ',',',runmodel,',', &
       &                       (int(lat_center(t)*10.0)),latmark,',',int((lon_center(t)*10.0)),lonmark, &
       &                       ',',int(wspd_max(t)),',',int(slp_min(t)),',','  ',',',0,',','   ',',',0,',',0,',',0,',',0, &
       &                       ',',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,',' ,'
               endif  
            endif  ! [(is_found(t))]
      end do

    500 format(A3,A1,I3,A1,A10,A1,A2,A1,A4,A1,I4,A1,A1,I5,A1,A1,I4,A1,I4,A1,A2,A1,I3,A1,A3,A1,I4,A1,I4,A1,I4,A1,I4,A1,12(A2)) 
...
!------ 3-D variables within serch radius rrr-------
      ...
      cix = ci2 - ci1 + 1
      cjx = cj2 - cj1 + 1
      !write(*,'(A5,1x,4(I3,1x))')'ci cj',ci1,ci2,cj1,cj2
...
!   c. then this is new center 
      ...
      do j = cj1,cj2
       ...
       enddo
      enddo
      eta_i_center(k) = i_weight / count_weight;
      eta_j_center(k) = j_weight / count_weight;
...
!!     enddo
     ...
     do k = 1, kx
     write(100,'(5F10.3)') eta_i_center(k),eta_j_center(k),eta_lon_center(k),eta_lat_center(k) 
     enddo
!-------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------  
         ...
         ! cylindrical 2-D variables
          ...
          !create netcdf file
           ...
           rcode=cg_add_1d_var(w2cfile,'radius','radius','km','Distance from vortex ceneter',RR)
           rcode=cg_add_1d_var(w2cfile,'theta','theta','degree','Azimuth angle',cg_ang)
           rcode=cg_add_1d_var(w2cfile,'bottom_top','bottom_top','','Eta_level',cg_k)
           !!! write 2d variable
