...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!   get_variable0d - subroutine that reads netcdf data for a 0    
...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine get_variable0d(fid, var, it, dat)

      character (len=*), intent(in) :: var
      integer, intent(in)           :: fid, it
      real, intent(out)             :: dat

      integer :: istart(1), iend(1), varid, rcode
      ...
      istart(1) = it
      iend(1) = 1
       print*,'varid',varid,it
      ! read the data
      rcode = nf_get_vara_real(fid, varid, istart, iend, dat)
       print*,'varid',dat

      if (rcode.ne.0) then 
        call netcdf_error('get_variable0d', var, rcode, 0, 0, 0, it)
      endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      call open_file (file, nf_nowrite, fid)
      rcode = nf_inq_varid ( fid, plot_var, id_var )
      rcode = nf_inq_var(fid,id_var,plot_var,ivtype,ndims,dimids,natts)
